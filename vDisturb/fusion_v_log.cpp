...
int		main(int	argc, char ** argv) {
	if (argc != 2) {
		std::cout << "usage: ./vLogFusion match_dir_path\nwith left.vDist.log et right.vDist.log in it" << std::endl;
		return 0;
	}
	std::ifstream			inLeft(argv[1] + std::string("/compressLeft.vDist.log")), inRight(argv[1] + std::string("/compressRight.vDist.log"));
	std::ofstream			ofs(argv[1] + std::string("/vDist.log"));
	std::string				str[3];
