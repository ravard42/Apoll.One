...
public:
			...
			void						printSettings( void ) {
				...
				std::cout << "SLICE IN SECONDE = " << SLICE_IN_SEC << std::endl;
			}
			...
			void						update( double radius, int f ) {
					...
					if (_f_start == 0 && radius >= THRESHOLD) {
						_f_start = f;
						_radius = radius;
						_log << std::endl << "\033[37m" << time(f) <<  "\033[0m\033[31m -> DISTURBANCE BEGINS\033[0m" << std::endl;
					}
						...
						_log << "\033[37m" << time(f) <<  "\033[0m\033[32m -> DISTURBANCE ENDS\033[0m" << std::endl << std::endl;
						_f_start = radius = 0;
					}
...
{
		if (argc != 2) {
			std::cout << "usage: ./findDisturb file.mp4" << std::endl;
			return 0;
		}

   	std::string									input(argv[1]);
		VideoCapture 								video(input);
		Disturb											disturb(input.replace(input.size() - 3, 3, "disturb.log"), video.get(CAP_PROP_FPS));
