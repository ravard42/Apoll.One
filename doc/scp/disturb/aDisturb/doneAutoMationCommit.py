...
def find_delay(time_pairs):
	...
	for i in range(len(time_pairs)):
		...
		if delta_t in t_diffs:
			...
			t_diffs[delta_t] = 1
	sync = max(t_diffs.values()) > threshold
	# if sync:
	# 	plt.plot(t_diffs.keys(), t_diffs.values())
	# 	plt.show()

	print(max(t_diffs.values()))
	...
	for s in range(20):
		for n in range(s+1):
			...
			if (not is_valid_slice(lvid, l) or not is_valid_slice(rvid, r)):
				continue
			offset1, offset2, sync = find_time_offset(lvid, rvid, l, r, durations=(duration, duration))
			print("check", sync)
